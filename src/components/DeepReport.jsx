  ...
  return (
    <div style={{ background:"#020209", minHeight:"100vh", fontFamily:"'Apple SD Gothic Neo',system-ui,sans-serif", color:"#e2e8f0" }}>
      ...
      <div style={{ padding:"18px 22px", borderBottom:"1px solid rgba(255,255,255,0.06)", background:"rgba(2,2,9,0.97)", display:"flex", alignItems:"center", gap:12, position:"sticky", top:0, zIndex:10 }}>
        ...
        <div style={{ flex:1 }}>
          <div style={{ fontSize:12, letterSpacing:4, color:`rgba(${ar},${ag},${ab},0.9)`, fontWeight:600 }}>AURA DEEP REPORT</div>
          <div style={{ fontSize:19, fontWeight:900, color:"#f1f5f9" }}>융합 아우라 심층 분석</div>
        </div>
      ...
      <div style={{ padding:"0 0 32px" }}>
        ...
        <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}}
          ...
          <div style={{ display:"flex", gap:20, alignItems:"flex-start", position:"relative" }}>
            ...
            <div>
              <div style={{ fontSize:13, letterSpacing:4, color:`rgba(${ar},${ag},${ab},0.85)`, fontWeight:600, marginBottom:7 }}>YOUR AURA TYPE</div>
              <div style={{ fontSize:26, fontWeight:900, color:"#c4b5fd", lineHeight:1.2, marginBottom:7 }}>{auraType.type}</div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:14 }}>
            {[
              ...
              <div key={i} style={{ borderRadius:10, padding:"14px 12px", background:`rgba(${ar},${ag},${ab},0.06)`, border:`0.5px solid rgba(${ar},${ag},${ab},0.2)`, textAlign:"center" }}>
                ...
                <div style={{ fontSize:24, fontWeight:900, color:`rgb(${ar},${ag},${ab})` }}>{p.gan}</div>
                <div style={{ fontSize:20, fontWeight:700, color:"#c4b5fd", marginTop:5 }}>{p.ji}</div>
                ...
                <div style={{ flex:1 }}>
                  ...
                  {cardRelation && (
                    <>
                      ...
                      <div style={{ marginTop:12, padding:"14px 16px", borderRadius:10, background:"rgba(236,72,153,0.06)", border:"1px solid rgba(236,72,153,0.18)", fontSize:16, color:"#e2e8f0", lineHeight:2.0 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {[
              ...
              <div key={i} style={{ borderRadius:11, padding:"16px 15px", background:"rgba(255,255,255,0.03)", border:"0.5px solid rgba(255,255,255,0.08)" }}>
                ...
                <div style={{ fontSize:13, letterSpacing:2, color:"#9ca3af", fontWeight:600, marginBottom:6 }}>{f.label}</div>
                <div style={{ fontSize:17, fontWeight:800, color:"#f1f5f9", marginBottom:8 }}>{f.title}</div>
                <div style={{ fontSize:15, color:"#b0a8c8", lineHeight:1.75, marginBottom:10 }}>{f.desc}</div>
        ...
        <Section title="2026 하반기 월별 운세" color={[ar,ag,ab]}>
          ...
          {monthlyFortune.filter(m=>m.peak).length > 0 && (
            <div style={{ marginTop:12, padding:"12px 14px", borderRadius:10, background:`rgba(${ar},${ag},${ab},0.06)`, border:`0.5px solid rgba(${ar},${ag},${ab},0.2)`, fontSize:15, fontWeight:700, color:`rgba(${ar},${ag},${ab},1)` }}>
        ...
        <Section title="나의 아우라 부적" color={[ar,ag,ab]}>
          <div style={{ display:"flex", gap:14, alignItems:"center", background:`rgba(${ar},${ag},${ab},0.05)`, border:`1px solid rgba(${ar},${ag},${ab},0.16)`, borderRadius:12, padding:"16px 18px" }}>
            ...
            <div>
              <div style={{ fontSize:13, letterSpacing:3, color:`rgba(${ar},${ag},${ab},0.9)`, fontWeight:700, marginBottom:5 }}>AURA TALISMAN</div>
              <div style={{ fontSize:19, fontWeight:900, color:"#f1f5f9", marginBottom:5 }}>{dominant.name} · {zodiacAnimal}의 부적</div>
              <div style={{ fontSize:15, color:"#9ca3af", lineHeight:1.6, marginBottom:12 }}>당신의 오행·타로를 기반으로 생성된<br />고화질 A4 인쇄용 맞춤 부적</div>
              <button style={{ padding:"7px 16px", background:`rgba(${ar},${ag},${ab},0.18)`, border:`1px solid rgba(${ar},${ag},${ab},0.4)`, borderRadius:20, color:`rgb(${ar},${ag},${ab})`, fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>↓ PNG 다운로드</button>
            </div>
          </div>
        </Section>
        ...
        <div style={{ margin:"22px 22px 0", padding:"13px 16px", borderRadius:10, background:"rgba(255,255,255,0.02)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <span style={{ fontSize:13, color:"#6b7280" }}>AURA © 2026 · 융합 분석 v2.6<br />{birthDate}생 맞춤 분석</span>
          <button style={{ padding:"7px 16px", background:`rgba(${ar},${ag},${ab},0.14)`, border:`0.5px solid rgba(${ar},${ag},${ab},0.3)`, borderRadius:20, color:`rgb(${ar},${ag},${ab})`, fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>↗ 공유하기</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{padding:"26px 22px 0"}}>
      <div style={{
        ...
        <span style={{
          display:"inline-block", width:4, height:18, borderRadius:4,
