  ...
  return (
    <div style={{ background:"#020209", minHeight:"100vh", fontFamily:"'Apple SD Gothic Neo', system-ui, sans-serif", color:"#e2e8f0", paddingBottom:140 }}>
      ...
      <div style={{ padding:"18px 22px 16px", borderBottom:"1px solid rgba(255,255,255,0.06)", background:"rgba(2,2,9,0.97)", display:"flex", alignItems:"center", gap:12, position:"sticky", top:0, zIndex:10 }}>
        <span style={{ fontSize:22, letterSpacing:3, color:`rgba(${ar},${ag},${ab},1)`, fontWeight:900 }}>AURA</span>
      ...
      <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}}
        ...
        <div style={{ display:"flex", gap:20, alignItems:"flex-start", position:"relative" }}>
          ...
          <div>
            <div style={{ fontSize:11, letterSpacing:4, color:`rgba(${ar},${ag},${ab},0.7)`, marginBottom:7 }}>YOUR AURA TYPE</div>
            <div style={{ fontSize:21, fontWeight:900, color:"#c4b5fd", lineHeight:1.2, marginBottom:6 }}>{auraType.type}</div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
          {[
            ...
            <div key={i} style={{ borderRadius:10, padding:"14px 12px", background:`rgba(${ar},${ag},${ab},0.06)`, border:`0.5px solid rgba(${ar},${ag},${ab},0.2)`, textAlign:"center" }}>
              ...
              <div style={{ fontSize:18, fontWeight:900, color:`rgb(${ar},${ag},${ab})` }}>{p.gan}</div>
              <div style={{ fontSize:16, fontWeight:700, color:"#c4b5fd", marginTop:4 }}>{p.ji}</div>
          ...
          <Section title="타로 · 오행 융합" color={[ar,ag,ab]}>
            <div style={{ display:"flex", gap:14, marginBottom:12 }}>
              ...
              <div style={{ flex:1 }}>
                ...
                {cardRelation && <div style={{ marginTop:10, fontSize:13, color:"#9ca3af", lineHeight:1.7 }}>{cardRelation.desc}</div>}
              </div>
            </div>
          </Section>
          <Divider />
      ...
      <Section title="분야별 운세" color={[ar,ag,ab]}>
        <div style={{ borderRadius:11, padding:"15px 14px", marginBottom:10, background:"rgba(255,255,255,0.03)", border:"0.5px solid rgba(255,255,255,0.08)" }}>
          ...
          <div style={{ fontSize:15, fontWeight:700, color:"#e2e8f0", marginBottom:6 }}>{domainFortune.love.title}</div>
          <div style={{ fontSize:13, color:"#9ca3af", lineHeight:1.6 }}>{domainFortune.love.desc}</div>
        ...
        <div style={{ position:"relative" }}>
          ...
          <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:8 }}>
            ...
            <span style={{ fontSize:14, color:"#9ca3af" }}>전체 결과에서 공개</span>
          </div>
        </div>
      </Section>
      ...
      <div style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:50, padding:"16px 20px 28px", background:"linear-gradient(to top, #020209 60%, transparent)" }}>
        <motion.button whileTap={{ scale:0.97 }} onClick={onPaid}
          style={{ width:"100%", padding:"17px 0", background:`linear-gradient(90deg, rgb(${ar},${ag},${ab}), rgba(${ar},${ag},${ab},0.75))`, border:"none", borderRadius:14, color:"#fff", fontSize:17, fontWeight:800, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8, fontFamily:"inherit", letterSpacing:1 }}>
