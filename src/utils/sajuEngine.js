...
export const OHANG_COLORS = {
  "목(木)":[34,197,94],"화(火)":[239,68,68],"토(土)":[245,158,11],"금(金)":[156,163,175],"수(Water)":[59,130,246],"수(水)":[59,130,246]
};

function getYearPillar(year) {
  const gi = ((year-4)%10+10)%10, ji = ((year-4)%12+12)%12;
...
function getDayPillar(year, month, day) {
  ...
  const gi=((jd-10)%10+10)%10, ji=((jd-10)%12+12)%12;
  return { gan:CHEONGAN[gi], ji:JIJI[ji], ganOhang:CHEONGAN_OHANG[gi], jiOhang:JIJI_OHANG[ji] };
}

function calcOhangDist(pillars) {
  const c={"목(木)":0,"화(火)":0,"토(土)":0,"금(金)":0,"수(水)":0};
  pillars.forEach(p=>{ if(p.ganOhang)c[p.ganOhang]=(c[p.ganOhang]||0)+1; if(p.jiOhang)c[p.jiOhang]=(c[p.jiOhang]||0)+1; });
  const total=Object.values(c).reduce((s,v)=>s+v,0);
  const COLORS={"목(木)":[34,197,94],"화(火)":[239,68,68],"토(土)":[245,158,11],"금(金)":[156,163,175],"수(水)":[59,130,246]};
  const TRAITS={"목(木)":"진취적이고 창의적인 에너지를 지녔습니다.","화(火)":"강렬한 열정과 카리스마로 주변을 이끄는 기운입니다.","토(土)":"깊은 포용력과 균형감으로 주변을 안정시킵니다.","금(金)":"날카로운 통찰과 결단력이 당신의 강점입니다.","수(水)":"깊은 직관과 유연한 사고로 본질을 꿰뚫습니다."};
  return Object.entries(c).map(([name,cnt])=>({ name, count:cnt, pct:Math.round(cnt/total*100), color:COLORS[name]||[139,92,246], trait:TRAITS[name]||"" })).sort((a,b)=>b.pct-a.pct);
}
...
const AURA_MAP = {
  ...
  "수(Water)_목(木)":"수목 생명형","수(Water)_화(火)":"수화 신비형","수(水)_목(木)":"수목 생명형","수(水)_화(火)":"수화 신비형",
  "수(Water)_토(土)":"수토 심연형","수(Water)_금(金)":"수금 직관형","수(水)_토(土)":"수토 심연형","수(水)_금(金)":"수금 직관형",
};
const AURA_DESC = {
  "목화 창조형":"열정과 성장이 폭발하는 창조적 영혼","목수 지성형":"지혜로 성장을 이끄는 탐구의 영혼","목토 안정형":"뿌리 깊은 나무처럼 흔들리지 않는 영혼","목금 결단형":"빠른 결단과 성장이 공존하는 영혼",
  "화목 열정형":"넘치는 생명력과 열정으로 빛나는 영혼","화토 리더형":"카리스마와 포용력을 겸비한 영혼","화금 개혁형":"강렬한 변화를 추구하는 선구자 영혼","화수 역동형":"상반된 에너지가 만드는 역동적 영혼",
  "토목 성장형":"안정된 기반 위에 성장을 꿈꾸는 영혼","토화 온기형":"따뜻함과 포용으로 주변을 밝히는 영혼","토금 신뢰형":"깊은 신뢰와 원칙으로 움직이는 영혼","토수 지혜형":"조용한 깊이로 세상을 읽는 현자형 영혼",
  "금목 도전형":"날카로운 결단으로 새 길을 여는 영혼","금화 강인형":"불 속에서 단련된 강철 같은 영혼","금토 원칙형":"흔들리지 않는 원칙과 기준의 영혼","금수 통찰형":"예리한 직관과 지혜를 겸비한 영혼",
  "수목 생명형":"깊은 지혜로 새 생명을 키우는 영혼","수화 신비형":"물과 불처럼 신비로운 이중성의 영혼","수토 심연형":"깊은 내면으로 세상을 품는 영혼","수금 직관형":"날카로운 직관과 지혜로 진실을 꿰뚫는 영혼",
};

function getAuraType(dist) {
  if(dist.length<2) return {type:"균형형 아우라",desc:"오행이 고르게 분포된 균형의 영혼",color:[139,92,246]};
  const key=`${dist[0].name}_${dist[1].name}`;
  const typeName=AURA_MAP[key]||`${dist[0].name} 중심형`;
  return {type:typeName, desc:AURA_DESC[typeName]||dist[0].trait, color:dist[0].color};
}
...
const REL_MAP = {
  ...
  "수(Water)_화(火)":{rel:"상극(相剋)",desc:"물이 불을 끄듯, 상반된 에너지가 역동을 만듭니다."},
...
function getMonthlyFortune(birthDate, dominant) {
  ...
  const kws=KW[dominant.name]||KW["수(Water)"]||KW["수(水)"];
...
function getDomainFortune(birthDate,dominant){
  const seed=new Date(birthDate).getTime();
...
function getCoreMessage(dominant,birthDate){
  ...
  const MSGS={
    ...
    "화(火)":[
      ...
      `불은 어둠을 밝히지만 동시에 스스로를 소모합니다. 당신의 화(火) 에너지가 지금 당신을 지치게 한다면, 내려놓아야 할 것이 있다는 신호입니다.\n\n포기가 아닙니다. 더 큰 불꽃을 위한 연료 절약입니다.`,
    ],
    "토(土)":[
      `대지의 토(土) 기운을 가진 ${age}세의 당신. 주변 모든 것을 품어안으려는 깊은 포용력의 소유자입니다.\n\n언제나 받아주는 역할을 해온 당신, 이제는 당신 자신을 위한 시간이 필요합니다. 스스로를 먼저 채워야 타인도 채울 수 있습니다.`,
    ...
    "수(水)":[
      ...
      `당신의 수(水) 기운은 직관과 통찰의 에너지입니다. 스스로도 설명하기 어려운 '느낌'으로 진실을 먼저 알아채는 당신.\n\n2026년 하반기, 그 직관을 믿으세요. 당신의 내면이 이미 답을 알고 있습니다.`,
    ],
  };
