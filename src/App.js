...
이 카드의 키워드 중 지금 당신의 마음에 가장 먼저 닿는 단어는 무엇인가요?`;
      ...
      setTimeout(() => {
        ...
        const finalText = finalResponse
          ? finalResponse + "\n\n✦ 분석이 완성되었습니다."
          : "✦ 분석이 완성되었습니다.\n\n당신의 아우라와 운명의 흐름이 선명하게 읽힙니다.";
        setMessages(prev => [...prev, {
          from: "ai",
          text: finalText,
          showResultBtn: true,   // 결과 보기 버튼 표시
          id: Date.now() + 2
        }]);
        setIsTyping(false);
        // 자동 이동도 유지 (버튼 못 봤을 경우 대비)
                  ...
                  <div style={{
                    ...
                    {msg.special === "birth_input" && !birthDate && (
                      <BirthInput onSubmit={(date) => {
                        ...
                        setTimeout(() => {
                          const flow = CHAT_FLOWS[chatType];
                          const isLastStep = chatStep + 1 >= flow.length - 1;
                          setMessages(prev => [...prev, {
                            from: "ai",
                            text: `✦ ${result.yearPillar.animal}띠 · ${date.replace(/-/g,".")}생

${result.dominant.name} 기운이 가장 강하게 흐르는 당신의 아우라가 선명해집니다.

${result.dominant.trait}`,
                            ohang: chatType !== "tarot",
                            birthDateData: date,
                            showResultBtn: isLastStep,
                            id: Date.now()
                          }]);
                      ...
                      <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.5 }}>
                        <motion.button
                          ...
                          style={{
                            ...
                            boxShadow: `0 0 24px rgba(${gr},${gg},${gb},0.3)`,
                          }}
                        >
                          ✦ 나의 아우라 결과 보기 →
                        </motion.button>
                        <div style={{ textAlign:"center", marginTop:8, fontSize:12, color:"#4b5563" }}>
                          분석이 완성되었습니다
                        </div>
                      </motion.div>
                    )}
